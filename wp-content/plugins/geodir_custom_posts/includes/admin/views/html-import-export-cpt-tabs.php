<?php
/**
 * Display the page to manage import/export cpt tabs.
 *
 * @since 2.3.3
	...
	<div class="gd-content-heading">
		...
		<div id="gd_ie_im<?php echo $section; ?>" class="metabox-holder accordion">
			<div class="card p-0 mw-100 border-0 shadow-sm" style="overflow: initial;">
				<div class="card-header bg-white rounded-top"><h2	class="gd-settings-title h5 mb-0 "><?php echo __( 'Import CPT Tabs', 'geodir_custom_posts' ); ?></h2></div>
				<div id="gd_ie_im_<?php echo $section; ?>" class="gd-hndle-pbox card-body gd-imex-box">
					...
					echo aui()->select(
							array(
								...
								'label'            => __( 'If tab already exists', 'geodir_custom_posts' ),
								'help_text'        => __( 'If the tab exists with id or combination of post_type, tab_name and tab_key in the CSV, you can either update the tab or it can be skipped.', 'geodir_custom_posts' ),
								'label_col'        => '3',
								...
								'options'          => array(
									...
									'update' => __( 'Update Tab', 'geodir_custom_posts'),
								),
								'select2'          => true,
		...
		<div id="gd_ie_ex<?php echo $section; ?>" class="metabox-holder accordion">
			<div class="card p-0 mw-100 border-0 shadow-sm" style="overflow: initial;">
				<div class="card-header bg-white rounded-top"><h2 class="gd-settings-title h5 mb-0"><?php echo __( 'Export CPT Tabs', 'geodir_custom_posts' ); ?></h2></div>
				<div id="gd_ie_ex_<?php echo $section; ?>" class="gd-hndle-pbox card-body">
					<input type="hidden" id="gd_chunk_size" value="10000">
					<?php
					echo aui()->select(
							array(
								'id'               => 'gd_post_type',
								'name'             => 'gd_imex[post_type]',
								'label'            => __( 'Post Type', 'geodir_custom_posts' ),
					...
					$items = GeoDir_CP_Admin_Import_Export::get_cpt_tabs( 'gd_place' );
					?>
					<div data-argument="gd_field" class="<?php echo ( $aui_bs5 ? 'mb-3' : 'form-group' ); ?> row aui-conditional-field" data-element-require="jQuery('#gd_post_type').val()!=''">
						<label for="gd_field_x" class="font-weight-bold fw-bold col-sm-3 col-form-label"><?php _e( 'Tabs', 'geodir_custom_posts' ); ?></label>
						<div class="col-sm-9 mt-2">
							...
							echo aui()->button(
								array(
									'type'    => 'a',
									'content' => __( 'Select All Tabs', 'geodir_custom_posts' ),
									'href'    => 'javascript:void(0)',
									'onclick' => "jQuery('.geodir-export-items').find('[type=\'checkbox\']').prop('checked', true);",
							...
							echo aui()->button(
								array(
									'type'    => 'a',
									'content' => __( 'Deselect All Tabs', 'geodir_custom_posts' ),
									'href'    => 'javascript:void(0)',
									'onclick' => "jQuery('.geodir-export-items').find('[type=\'checkbox\']').prop('checked', false);",
									'class'   => 'btn btn-sm btn-secondary'
								)
							);
							?>
							</div>
							<div class="border rounded px-3 pt-2 pb-1 scrollbars-ios" style="max-height:260px;overflow-y:auto;overflow-x:hidden;"><div class="row geodir-export-items">
							<?php
							foreach ( $items as $key => $item ) {
								?><div class="col-sm-6 mb-1"><?php
								echo aui()->input(
										array(
											'id'               => 'gd_field_'. $key,
											'name'             => 'gd_imex[tabs][]',
											'type'             => 'checkbox',
											'value'            => (int) $item->id,
											'label'            => stripslashes( $item->tab_name ) . '<div class="d-inline text-muted"> - ' . $item->tab_key . ' (' . $item->tab_type . ')</div>',
											'label_type'       => 'hidden',
											'no_wrap'          => true
										)
									);
