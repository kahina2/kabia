<?php
/**
 * Display the page to manage import/export custom fields.
 *
 * @since 2.2.1
	...
	<div class="gd-content-heading">
		...
		<div id="gd_ie_im<?php echo $section; ?>" class="metabox-holder accordion">
			<div class="card p-0 mw-100 border-0 shadow-sm" style="overflow: initial;">
				<div class="card-header bg-white rounded-top"><h2	class="gd-settings-title h5 mb-0 "><?php echo __( 'Import Custom Fields', 'geodir_custom_posts' ); ?></h2></div>
				<div id="gd_ie_im_<?php echo $section; ?>" class="gd-hndle-pbox card-body gd-imex-box">
					...
					echo aui()->select(
							array(
								...
								'label'            => __( 'If Field with field_key Exists', 'geodir_custom_posts' ),
								'help_text'        => __( 'If the custom field with field_key exists in the CSV, you can either update the custom field or it can be skipped.', 'geodir_custom_posts' ),
								'label_col'        => '3',
								...
								'options'          => array(
									...
									'update' => __( 'Update Custom Field', 'geodir_custom_posts'),
								),
								'select2'          => true,
		...
		<div id="gd_ie_ex<?php echo $section; ?>" class="metabox-holder accordion">
			<div class="card p-0 mw-100 border-0 shadow-sm" style="overflow: initial;">
				<div class="card-header bg-white rounded-top"><h2 class="gd-settings-title h5 mb-0"><?php echo __( 'Export Custom Fields', 'geodir_custom_posts' ); ?></h2></div>
				<div id="gd_ie_ex_<?php echo $section; ?>" class="gd-hndle-pbox card-body">
					...
					$fields = GeoDir_CP_Admin_Import_Export::get_custom_fields( 'gd_place' );
					?>
					<div data-argument="gd_field" class="<?php echo ( $aui_bs5 ? 'mb-3' : 'form-group' ); ?> row aui-conditional-field" data-element-require="jQuery('#gd_post_type').val()!=''">
						<label for="gd_field_x" class="font-weight-bold fw-bold col-sm-3 col-form-label"><?php _e( 'Fields', 'geodir_custom_posts' ); ?></label>
						<div class="col-sm-9 mt-2">
							...
							echo aui()->button(
								array(
									'type'    => 'a',
									'content' => __( 'Select All Fields', 'geodir_custom_posts' ),
									'href'    => 'javascript:void(0)',
									'onclick' => "jQuery('.geodir-export-fields').find('[type=\'checkbox\']').prop('checked', true);",
							...
							echo aui()->button(
								array(
									'type'    => 'a',
									'content' => __( 'Deselect All Fields', 'geodir_custom_posts' ),
									'href'    => 'javascript:void(0)',
									'onclick' => "jQuery('.geodir-export-fields').find('[type=\'checkbox\']').prop('checked', false);",
									'class'   => 'btn btn-sm btn-secondary'
								)
							);
							?>
							</div>
							<div class="border rounded px-2 scrollbars-ios" style="max-height:260px;overflow-y:auto;overflow-x:hidden;"><div class="row geodir-export-fields">
							<?php
							foreach ( $fields as $key => $field ) {
								...
								echo aui()->input(
										array(
											...
											'type'             => 'checkbox',
											'value'            => (int) $field->id,
											'label'            => $field->htmlvar_name . ' ( ' . $field->admin_title . ' ) ',
											'label_type'       => 'hidden',
											'no_wrap'          => true
										)
									);
