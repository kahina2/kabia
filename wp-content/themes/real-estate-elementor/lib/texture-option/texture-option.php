...
class real_estate_elementor_theme_option{
...
function menu_tab() {
    $menu_title = esc_html__('Real State Elementor Options', 'real-estate-elementor');
    add_theme_page( esc_html__( 'Real Estate Elementor', 'real-estate-elementor' ), $menu_title, 'edit_theme_options', 'texture_started',array($this,'tab_page'));
...
wp_enqueue_script( 'real-estate-elementor-admin-load', get_template_directory_uri() . '/lib/texture-option/assets/js/texture-options.js', array('jquery', 'updates' ), true );
...
$data = apply_filters(
    ...
    wp_localize_script( 'real-estate-elementor-admin-load', 'THAdmin', $data); 
...
function tab_constant(){
    ...
    $tab_array = array();
    $tab_array['header'] = array('theme_brand' => __('wptexture','real-estate-elementor'),
    'theme_brand_url' => esc_url($themedata->get( 'AuthorURI' )),
    'welcome'=>sprintf(esc_html__('Welcome To %1s Theme', 'real-estate-elementor'), esc_html($themedata->get( 'Name' ),'real-estate-elementor'), $themedata->get( 'Version' ) ),
    'welcome_desc' => esc_html($themedata->get( 'Name' ).' is an eCommerce WordPress theme specially made for shopping websites.', 'real-estate-elementor' ),
...
if(empty($pages)){
      $post_id = wp_insert_post(array (
       'post_type' => 'page',
       'post_title' => __('Home Page','real-estate-elementor'),
       'post_content' => '',
       'post_status' => 'publish',
     ...
     /*
      ...
      if ( ! current_user_can( 'install_plugins' ) || ! isset( $_POST['init'] ) || ! $_POST['init'] ) {
        wp_send_json_error(
          array(
            'success' => false,
            'message' => __( 'No plugin specified', 'real-estate-elementor' ),
          )
        );
      }
      ...
      wp_send_json_success(
        array(
          'success' => true,
          'message' => __( 'Plugin Successfully Activated', 'real-estate-elementor' ),
        )
      );
...
function plugin_install_button($plugin){
            ...
            <h4>'.esc_html( $plugin['plugin_name'] ). ' </h4><a class="plugin-detail thickbox open-plugin-details-modal" href="'.esc_url( $plugin['detail_link'] ).'">'.esc_html__( 'Details & Version', 'real-estate-elementor' ).'</a>
            </div>';
        ...
        foreach($recommend_plugins[0] as $slug=>$plugin){
            ...
            $button_class = 'install-now button '.$slug;
             ...
             if ( is_plugin_active( $plugin_init ) ) {
                   ...
                   $button_txt = esc_html__( 'Activated', 'real-estate-elementor' );
                   $detail_link = $install_url = '';
                }

            if ( ! is_plugin_active( $plugin_init ) ){
                    $button_txt = esc_html__( 'Install Now', 'real-estate-elementor' );
                    if ( ! $status ) {
                        ...
                        $button_txt = esc_html__( 'Activate Now', 'real-estate-elementor' );
                    }
                }
...
$boj = new real_estate_elementor_theme_option(); ?>